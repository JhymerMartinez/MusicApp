    ...
    <div className="flex-1 flex flex-row justify-between items-center">
      ...
      <div className="flex-1 flex flex-col justify-center mx-3">
        <Link to={`/songs/${song?.id}`}>
          <p className="text-xl font-bold text-white hover:text-emerald-200">{song?.title}</p>
        </Link>
        <Link to={`/artists/${song?.artist?.id}`}>
          <p className="text-base text-gray-300 mt-1 hover:text-emerald-200">{song?.artist?.name}</p>
        </Link>
      </div>
    </div>
...
const TopPlay = () => {
  ...
  const { data } = useGetTopSongsByGenreQuery({
    genreId: 132, // POP
  });
    ...
    <div ref={divRef} className="xl:ml-6 ml-0 xl:mb-0 mb-6 flex-1 xl:max-w-[500px] max-w-full flex flex-col">
      <div className="w-full flex flex-col">
        <div className="flex flex-row justify-between items-center">
          ...
          <Link className="text-gray-300 text-base cursor-pointer hover:text-emerald-200" to="/top-songs">See more</Link>
        </div>
      ...
      <div className="w-full flex flex-col mt-8">
        <div className="flex flex-row justify-between items-center">
          ...
          <Link className="text-gray-300 text-base cursor-pointer hover:text-emerald-200" to="/top-artists">See more</Link>
        </div>
